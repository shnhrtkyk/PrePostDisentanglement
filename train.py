...
def train(args):
    ...
    while True:
        ...
        for domA_img, domB_img in zip(domA_loader, domB_loader):
            ...
            print(A_separate.size())
            A_encoding = torch.cat([A_common, A_separate], dim=1)
            print(A_encoding.size())
            
            B_common = e1(domB_img)
            print(B_common.size())
            B_separate = e2(domA_img)
            print(B_separate.size())
            B_encoding = torch.cat([B_common, B_separate], dim=1)
            print(B_encoding.size())
            
            AB_encoding = torch.cat([A_common, B_common], dim=1)
            print(AB_encoding.size())
            BA_encoding = torch.cat([B_common, A_common], dim=1)
            print(BA_encoding.size())
            ...
            loss = mse(A_decoding, domA_img) + mse(B_decoding, domB_img) +  mse(AB_decoding, domA_img) + mse(BA_decoding, domB_img)
